...
_ty = lambda text: _fc(Fore.LIGHTYELLOW_EX, text)
_tg = lambda text: _fc(Fore.LIGHTGREEN_EX, text)
...
class YTSum:
    ...
    def download_yt_audio(self, url: str, output_folder: Path = None, filename: str = None):
        ...
        if f_path.exists():
            print(f"{_tg('Found')} {_ty(f_path)}, {_tg('skipping download...')}")
        else:
            print(f"{_tg('>> Downloading audio from ')}{url}{_tg('...')}")
            (yt.streams
             .filter(only_audio=True, file_extension="mp4")
             .order_by('abr')
             .last()
             .download(output_path=str(output_path), filename=f"{f_name}.mp4"))
            # print(_tg(f"Downloaded audio to {output_path}/{f_name}.mp4"))
    ...
    def transcribe(self, audio_file: Path = None, use_cache: bool = True, verbose: bool = False):
        ...
        :param verbose: Flag to print the transcription
        ...
        if use_cache:
            ...
            if cache_path.exists():
                print(f"{_tg('Found')} {_ty(cache_path)}, {_tg('skipping transcription...')}")
                self.transcription = cache_path.read_text()
                if verbose:
                    pprint(f"{_ty('Transcription')}: {_tg(self.transcription)} \n {_ty('---------------------')} \n ")
                return self

        print(f"{_tg('>> Transcribing audio to text ...')}")
        ...
        if verbose:
            pprint(f"{_ty('Transcription')}: {_tg(transcription)} \n {_ty('---------------------')} \n ")
...
""",
        ...
        print(f"{_tg('>> Summarizing transcription ...')}")
    ...
    def print(self):
        ...
        for txt in [self.summary, self.transcription]:
            if txt is not None:
                ...
                return self
        print(_ty("No summary found. Please run summarize or transcribe first."))
        return self
